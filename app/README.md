...
- **`/materiale`** — stoc: nume, cod/culoare, cantitate, unitate, **preț total** (pentru toată
  cantitatea, nu per bucată), furnizor, prag minim (marchează "stoc redus" când cantitatea
  scade sub prag). Confirmare de ștergere cu design propriu, nu popup-ul urât al browserului.
- **`/masini`** — lucrări: număr înmatriculare, **model mașină**, dată, descrierea lucrării,
  materiale folosite (cu cantitate) și **totalul calculat automat** pentru lucrare (pe baza
  prețului materialelor consumate). Lucrările pot fi **editate** (creion) — la salvare,
  consumul vechi de stoc se anulează și se aplică cel nou. Ștergerea unei lucrări returnează
  automat stocul consumat.
- Navigare cu tab-uri sus (Dashboard / Materiale / Mașini), fără parolă de acces.

## Dacă ai deja creat baza de date (schema.sql rulat anterior)

Proiectul a primit 2 coloane noi (`model` la mașini, `cost` la materialele folosite per
lucrare). Rulează `migration.sql` în Supabase → SQL Editor, o singură dată — nu îți afectează
datele existente, doar adaugă coloanele lipsă.
